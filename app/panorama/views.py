...
from .forms import PanoramaRequestForm
...
def download_and_delete_file(request, report_request_id):
    ...
    report_request = PanoramaRequest.objects.get(id=report_request_id)
...
def delete_old_requests(request_user):
    ...
    old_requests = PanoramaRequest.objects.filter(created__lt=timezone.now() - timedelta(hours=24))
    for request in old_requests:
        request.file.delete()
        request.delete()

    
    return PanoramaRequest.objects.filter(profile=request_user).order_by('-created')
...
def index(request, uuid=None):
    ...
    if request.method == 'POST':
        form = PanoramaRequestForm(request.POST, request.FILES)
        ...
        if form.is_valid():
            ...
            try:
                ...
                # Add form error
                return render(request, 'panorama/index.html', {'form': form, 'error': 'Could not connect to Canvas. Invalid token or URL.'})
            ...
            try:
                ...
                return render(request, 'panorama/index.html', {'form': form, 'error': 'Could not find user. Invalid token or URL.'})
            ...
            profile_obj, created = PanoramaProfile.objects.get_or_create(
                email=email,
                defaults={
                    'name': name,
            ...
            try:
                ...
                report_request = PanoramaRequest(profile=profile_obj)
                report_request.from_date = from_date
                report_request.to_date = to_date
                report_request.save()
            except:
                return render(request, 'panorama/index.html', {'form': form, 'error': 'Could not find course or assignment. Check course and assignment ID.'})
            ...
            return HttpResponseRedirect(reverse('panorama:index-uuid', args=[uuid]))
    ...
    if uuid:
        try:
            profile = PanoramaProfile.objects.get(uuid=uuid)
    ...
    context['form'] = PanoramaRequestForm()

    return render(request, 'panorama/index.html', context)
