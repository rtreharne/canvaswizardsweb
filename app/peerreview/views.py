...
def index(request, uuid=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # Add form error
                return render(request, 'peerreview/index.html', {'form': form, 'error': 'Could not connect to Canvas. Invalid token or URL.'})
            ...
            try:
                ...
                return render(request, 'peerreview/index.html', {'form': form, 'error': 'Could not find user. Invalid token or URL.'})
            ...
            try:
                ...
                return render(request, 'peerreview/index.html', {'form': form, 'error': 'Could not find course or assignment. Check course and assignment ID.'})
            
            generate_report.delay(CANVAS_URL, CANVAS_TOKEN, report_request.id, course_id, assignment_id, encryption_password)

            return HttpResponseRedirect(reverse('peerreview:index-uuid', args=[uuid]))
    ...
    return render(request, 'peerreview/index.html', context)
