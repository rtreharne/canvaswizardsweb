...
def generate_report(CANVAS_URL, CANVAS_TOKEN, report_request_id, from_date, to_date, encryption_password=None):
    ...
    report_request = OverviewRequest.objects.get(pk=report_request_id)

    # Overview_report functions here
    ...
    report = overview_report.generate_report(CANVAS_URL, CANVAS_TOKEN, from_date, to_date)
    ...
    file_name = f"{timestamp}_overview_report.xlsx"
    output = io.BytesIO()
